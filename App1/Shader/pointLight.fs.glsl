...
// cook torrance BRDF
vec3 BRDF_CookTorrance(Fragment fragment, vec3 cameraPos, vec3 lightPos)
{
	vec3 viewDir = normalize(cameraPos - fragment.worldPos);
	vec3 lightDir = normalize(lightPos - fragment.worldPos);
...
void main()
{
	...
	vec3 brdf = BRDF_CookTorrance(fragment, cameraPosition, lightPosition);
